...
- `prompt_banks/`  
  Raw JSONL prompt banks used for controlled arity, nonce-predicate, query-swap, and multi-template controls.

- `data/reviewer_rescue/`  
  Reviewer-rescue prompt-control banks generated by `analysis/build_reviewer_rescue_controls.py`, including the raw multi-template arity bank.

- `analysis/` and `scripts/`  
  Public runner and prompt-control scripts needed to rebuild and run the reviewer-rescue controls.
