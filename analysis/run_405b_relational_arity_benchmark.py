#!/usr/bin/env python3
"""405B controlled relational-arity benchmark.
...
from analysis.run_405b_gpqa_highk_depth_audit import (  # noqa: E402
...
DEFAULT_LAYERS = [20, 30, 40, 60, 80, 90]
...
def analyze_cache(args: argparse.Namespace) -> None:
    ...
    for pos, payload in enumerate(iter_cached_payloads(cache_dir, layers=layers, prompt_ids=prompt_ids), start=1):
        item = payload.get("item") or {}
        arity = int(item.get("relation_arity") or 0)
        ...
        for layer in layers:
            ...
            proj = deterministic_projection(hidden.shape[1], int(args.proj_dim), int(args.projection_seed))
            x_proj = hidden @ proj
            ...
            for k in ranks:
                ...
                selector_prefixes = build_selector_prefixes(
                    ...
                    norms=norms,
                    k=int(k),
                    max_budget=int(args.control_tuple_budget),
                    n_candidates=int(args.n_candidates),
                    n_hub_tokens=int(args.n_hub_tokens),
                ...
                selector_map = {
                    ...
                    "degree_salience": selector_prefixes.get("degree_salience", [])[: int(args.control_tuple_budget)],
                    "avg_pair_attention": selector_prefixes.get("avg_pair_attention", [])[: int(args.control_tuple_budget)],
                    "k_clique_all_pairs_attention": selector_prefixes.get("k_clique_all_pairs_attention", [])[: int(args.control_tuple_budget)] if int(k) >= 4 else [],
                }
    ...
    rows_path = results_dir / "405b_relational_arity_rows.csv"
    stats_path = results_dir / "405b_relational_arity_stats.csv"
    rows_df.to_csv(rows_path, index=False)
    stats_df = summarize_arity(rows_df)
    stats_df.to_csv(stats_path, index=False)
    plot_arity_heatmap(stats_df, figures_dir / "fig_405b_relation_arity_by_k_heatmap.png")
    plot_ground_truth_vs_controls(stats_df, figures_dir / "fig_405b_ground_truth_arguments_vs_controls.png")
    write_arity_summary(stats_df, results_dir / "405b_relational_arity_summary.md")
...
def plot_arity_heatmap(stats_df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_title("405B controlled relational-arity: best selector entropy gap")
...
def write_arity_summary(stats_df: pd.DataFrame, out_path: Path) -> None:
    lines = [
        "# 405B Relational-Arity Benchmark Summary",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-cache-dir", default=str(ROOT / "results" / "cache_405b_relational_arity_benchmark"))
    ...
    p.add_argument("--control-tuple-budget", type=int, default=200)
