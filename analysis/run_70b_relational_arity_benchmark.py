#!/usr/bin/env python3
"""70B controlled relational-arity benchmark.
...
from analysis.run_70b_gpqa_highk_depth_audit import (  # noqa: E402
...
DEFAULT_LAYERS = [20, 30, 40, 50, 55, 60, 70]
...
def analyze_cache(args: argparse.Namespace) -> None:
    ...
    rows_path = results_dir / "70b_relational_arity_rows.csv"
    stats_path = results_dir / "70b_relational_arity_stats.csv"
    rows_df.to_csv(rows_path, index=False)
    stats_df = summarize_arity(rows_df)
    stats_df.to_csv(stats_path, index=False)
    plot_arity_heatmap(stats_df, figures_dir / "fig_70b_relation_arity_by_k_heatmap.png")
    plot_ground_truth_vs_controls(stats_df, figures_dir / "fig_70b_ground_truth_arguments_vs_controls.png")
    write_arity_summary(stats_df, results_dir / "70b_relational_arity_summary.md")
...
def plot_arity_heatmap(stats_df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_title("70B controlled relational-arity: best selector entropy gap")
...
def write_arity_summary(stats_df: pd.DataFrame, out_path: Path) -> None:
    lines = [
        "# 70B Relational-Arity Benchmark Summary",
        "",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--output-cache-dir", default=str(ROOT / "results" / "cache_70b_relational_arity_benchmark"))
