#!/usr/bin/env python3
"""70B relational-arity diagonal-dominance audit.
...
from analysis.run_70b_relational_arity_benchmark import (  # noqa: E402
    DEFAULT_LAYERS,
    DEFAULT_RANKS,
    entropy_from_minors,
)
...
def plot_heatmap(best_df: pd.DataFrame, path: Path) -> None:
    ...
    fig.suptitle("70B arity x rank diagonal-enrichment audit")
...
def write_summary(best_df: pd.DataFrame, tests_df: pd.DataFrame, path: Path) -> None:
    lines = [
        "# 70B Arity Diagonal-Dominance Summary",
        "",
        "This audit recomputes off-diagonal tuple sets from the saved 70B arity cache.",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--cache-dir", default=str(ROOT / "results" / "cache_70b_relational_arity_benchmark"))
...
def main() -> None:
    ...
    csv_path = results_dir / "70b_arity_diagonal_dominance.csv"
    prompt_path = results_dir / "70b_arity_diagonal_dominance_prompt_rows.csv"
    fig_path = figures_dir / "fig_70b_arity_x_rank_diagonal_heatmap.png"
    summary_path = results_dir / "70b_arity_diagonal_dominance_summary.md"
