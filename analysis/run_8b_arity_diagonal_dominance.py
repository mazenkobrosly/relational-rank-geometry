#!/usr/bin/env python3
"""8B relational-arity diagonal-dominance audit.
...
def plot_heatmap(best_df: pd.DataFrame, path: Path) -> None:
    ...
    fig.suptitle("8B arity x rank diagonal-enrichment audit")
...
def write_summary(best_df: pd.DataFrame, tests_df: pd.DataFrame, path: Path) -> None:
    lines = [
        "# 8B Arity Diagonal-Dominance Summary",
        "",
        "This audit recomputes off-diagonal tuple sets from the saved 8B arity cache.",
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--cache-dir", default=str(ROOT / "results" / "cache_8b_relational_arity_benchmark"))
...
def main() -> None:
    ...
    csv_path = results_dir / "8b_arity_diagonal_dominance.csv"
    prompt_path = results_dir / "8b_arity_diagonal_dominance_prompt_rows.csv"
    fig_path = figures_dir / "fig_8b_arity_x_rank_diagonal_heatmap.png"
    summary_path = results_dir / "8b_arity_diagonal_dominance_summary.md"
